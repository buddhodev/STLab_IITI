...
export default function StlabWixsiteHub({ activeTab, setActiveTab }: StlabWixsiteHubProps) {
  ...
  const [pubFilter, setPubFilter] = React.useState<"all" | "dissertations" | "patents" | "other">("all");
  ...
  const [talkFilter, setTalkFilter] = React.useState<"all" | "invited" | "live">("all");
                    ...
                    <div className="space-y-3 flex-1 font-sans">
                      ...
                      <div>
                        <h4 className="text-lg sm:text-xl font-extrabold text-[#0c2340] leading-snug group-hover:text-st-orange transition-colors">
                          Double Victory at Best Idea Presentation Competition 2026!
                        </h4>
                        <p className="text-xs text-slate-650 mt-1.5 leading-relaxed font-light">
                          We are proud to share that STL Lab members <span className="font-bold text-slate-805">Abhishek D Kalbande (First Prize)</span> and <span className="font-bold text-slate-805">Buddhodev Ghosh (Joint-Second Prize)</span> triumphed at the Best Idea Presentation Competition.
                        </p>
                      </div>
        ...
        {activeTab === "publications" && (
          <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="space-y-1">
                <h3 className="text-xl font-extrabold text-[#0d56a6] uppercase tracking-tight">Publications & Intellectual Property</h3>
            ...
            <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-100 pb-3">
              ...
              >
                All Publications ({DISSERTATIONS.length + PATENTS.length + OTHER_PUBLICATIONS.length})
              </button>
              <button
              ...
              >
                📄 Journal & Book Chapters ({OTHER_PUBLICATIONS.length})
              </button>
            </div>
              ...
              {(pubFilter === "all" || pubFilter === "other") && (
                <div className="space-y-3 pt-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    }).length === 0 && pubFilter === "other" && (
                      <p className="text-xs text-slate-400 font-mono italic">No matching academic papers found.</p>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xs">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center">
                ...
                <div className="flex flex-wrap gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200/80 self-start">
                  {(["all", "invited", "live"] as const).map((filterVal) => {
                    const labelMap = {
                      all: "All Talks",
                    ...
                    const count = filterVal === "all" 
                      ? PI_TALKS.length 
                      : PI_TALKS.filter(p => p.category === filterVal).length;
              ...
              {(() => {
                const filteredTalks = PI_TALKS.filter((talk) => {
                  const matchesFilter = talkFilter === "all" || talk.category === talkFilter;
